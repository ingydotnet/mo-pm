...
In the past, Mo supported multiple inheritance. This was consider suboptimal
for a couple reasons. MI is generally frowned upon in Perl. It has problems
that are better solved by roles. It also makes the BUILD call sequence much
more difficult, which makes Moose compatibility hard. For this reason, we
removee MI as a Mo feature. As a result, the code became much simpler.
...
If neither C<default> nor C<build> is requested, C<has> will return a
...
This is a list of eveything else.

=over

=item Why not Moose?

The sad fact of Perl 5 is that OO isn't built in.

Moose not only added OO, it took OO to a new level. It is indeed a postmodern
system. Unfortunately this comes at a (often hefty) performance price.
...
other OO base modules like Spiffy and Gloom. There are things about those that
